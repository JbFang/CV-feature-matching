...
def compute_feature_distances(features1, features2):
    ...
    if n < m:
        for i in range(n):
            dists[i, :] = np.sqrt(\
                np.sum(np.abs(features1[i]-features2)**2, axis=-1))
    else:
        for j in range(m):
            dists[:, j] = np.sqrt(\
                np.sum(np.abs(features1-features2[j])**2, axis=-1))
...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    for i in range(len(features1)):
        dist = dists[i]
        m1, m2 = np.argsort(dist)[0:2]
        
        NNDR = dists[i][m1] / dists[i][m2]
        if NNDR < threshold:
            matches.append([i, m1])
            confidences.append(dists[i][m1])
...
def pca(fvs1, fvs2, n_components= 24):
    ...
    red_u = u[:, :n_components]
    red_v = v[:n_components]

    data_transformed = np.dot(data, red_v.T)
    
    reduced_fvs1 = data_transformed[:len(fvs1), :]
    reduced_fvs2 = data_transformed[len(fvs1):, :]
...
def accelerated_matching(features1, features2, x1, y1, x2, y2):
    ...
    nbrs = NearestNeighbors(n_neighbors = 2, algorithm = 'kd_tree').fit(features2)
    dists, indices = nbrs.kneighbors(features1)
    
    matches = []
    confidences = []
    for i in range(len(dists)):
        NNDR = dists[i][0] / dists[i][1]

        if NNDR < threshold:
            matches.append([i, indices[i][0]])
            confidences.append(dists[i][0])
    matches = np.array(matches)
    confidences = np.array(confidences)
    #############################################################################
    #                             END OF YOUR CODE                              #
